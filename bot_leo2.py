...
#(in modo che se hai già messo studente poi non può mettere docente)
...
async def start_command(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("Docente", callback_data="opzione_2")],
        ]),
    )
    """
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_msg = "Ecco una lista dei comandi attualmente disponibili su questo bot:\n\n"\
        ...
        "- /consegne: Visualizza gli ID degli studenti che hanno effettuato consegne nel giorno corrente\n"\
        "- /leggi ID_studente: Visualizza le foto caricate dagli studenti nel giorno corrente\n"\
        "- /end: Interrompe il comando attualmente in esecuzione e termina la conversazione\n"
...
url = "https://www.dmi.unipg.it/dipartimento/rubrica?categoria=DOC&lettera=&pagina="
HEADERS : final = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3"
}
response = requests.get(url, headers=HEADERS)

if response.status_code == 200: #richiesta ha avuto successo
    soup = BeautifulSoup(response.text, "html.parser") #analizzo contenuto HTML della pagina web
    #div = soup.find("div", id="dwm").get_text(strip=True) #trovo testo contenuto nel div con ID "dwn" e rimuovo spazi iniziali e finali
...
# Gesione della risposta alla richiesta dello status (come faccio a sapere che è solo dello status?)
def handle_response_int(text: int,update: Update) -> str:
    #processed: str = text.lower()  #trasforma il testo in caratteri minuscoli
    user_id = get_user_id(update)
    #Le tre istruzioni if sotto verificano se alcune parole chiave specifiche sono presenti nel testo processato.
...
# Gestione della risposta dal punto di vista del terminale (per il debugging)
# e invio della risposta (se tutto va bene)
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):

    # informazioni del tipo di messaggio ricevuto
    message_type: str = update.message.chat.type #es: "private" per una chat privata con il bot
    text: str = update.message.text

    # utile per debugging
    print(f'User ({update.message.chat.id}) in {message_type}: "{text}"') #f sta per formatted string
    #stampa le informazioni sull'utente e sul messaggio ricevuto nella console
    digit = remove_spaces(text)

    #gestione risposta a status (vedi sopra), es. "scrivi il tuo id"
    if digit.isdigit():
    ...
    print('Bot:', response)
    await update.message.reply_text(response) #invia la risposta


# Log errors (utile per il debugging CREDO)
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler('consegne', consegne_command))
